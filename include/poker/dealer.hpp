...
public:
    ...
    auto player_state(seat_index) const POKER_NOEXCEPT -> detail::round::player;
...
inline auto dealer::player_state(seat_index s) const POKER_NOEXCEPT -> detail::round::player {
    POKER_DETAIL_ASSERT(betting_round_in_progress(), "Betting round must be in progress");

    return _betting_round.player_state(s);
}
